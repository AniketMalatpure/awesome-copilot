...
| **Docker/Containers** | Running as root, no `USER` directive, host mounts, unsigned images, `--privileged` flag, Docker socket (`/var/run/docker.sock`) mount, `--security-opt seccomp=unconfined`, `--security-opt apparmor=unconfined` | Container hardening + escape prevention |
...
| **Kubernetes** | No NetworkPolicy, no PodSecurityPolicy/Standards, no resource limits, RBAC gaps, `hostPath` volume mounts, `hostPID: true`, `hostNetwork: true`, `hostIPC: true`, `automountServiceAccountToken: true` without need, `cluster-admin` ClusterRoleBinding to service accounts, missing admission controller (OPA/Gatekeeper/Kyverno), missing `securityContext.seccompProfile` (should be `RuntimeDefault`), `allowPrivilegeEscalation: true`, missing AppArmor annotations, Workload Identity Federation misconfiguration (wildcarded audience in `azure.workload.identity` annotations, missing issuer URL validation — see Cloud IAM row for IaC-level identity signals) | Network segmentation + privilege escalation + identity |
...
| **CI/CD Pipelines** | Secrets in logs, no artifact signing, mutable dependencies, script injection, no SBOM generation (`syft`, `cyclonedx`, `trivy sbom` absent), no SLSA provenance attestation (`slsa-github-generator`, `cosign attest`, `in-toto` absent), unpinned GitHub Actions (using `@main` instead of `@sha`), no lockfile (`package-lock.json`, `go.sum`, `Pipfile.lock` absent), image refs using `:latest` instead of `@sha256:` digest | Supply chain + provenance |
| **REST APIs** | Missing auth, no rate limiting, verbose errors, no input validation, SSRF (user-controlled URLs passed to `HttpClient`, `http.Get()`, `requests.get()`, `fetch()`, `urllib` without allowlist validation — check for URL parameters, redirect endpoints, webhook URLs, proxy endpoints that could reach internal services or cloud metadata endpoints like `169.254.169.254`) | Auth + injection + SSRF |
...
| **Audit/Logging** | No security event logging, log injection, no tamper protection. **Must verify logging of:** authentication failures, privilege escalation attempts, data access events, admin/operator actions, token issuance/revocation, configuration changes, API access from unexpected sources. Flag absence of structured security logging framework (Serilog (.NET), SLF4J/Logback (Java), winston/pino (Node.js), zap/zerolog (Go)) for security events | Monitoring gaps + repudiation |
| **Cloud IAM & IaC** | Overly broad role assignments: Azure Owner (`8e3af657-*`) or Contributor (`b24988ac-*`) in ARM/Bicep/Terraform without condition blocks, AWS `AdministratorAccess` or `PowerUserAccess` in CloudFormation/Terraform, GCP `roles/owner` or `roles/editor`. Wildcard resource access (`"*"`). Static service principal credentials instead of managed identity (`ManagedIdentityCredential` vs `ClientSecretCredential` / hardcoded connection strings with keys — see Kubernetes row for manifest-level WIF signals) | Privilege escalation + credential exposure |
| **Deserialization / Parsers** | Unsafe deserialization: `pickle.loads()`, `yaml.load()` without `SafeLoader`, `XMLReader` / `XmlDocument` without DTD disabled (`DtdProcessing.Prohibit`), `BinaryFormatter`, `ObjectInputStream.readObject()`, `JSON.parse()` + `eval()`. Archive extraction without path validation (zip slip: `ZipFile.extractall()`, `tar.extractall()` without member filtering). Schema-less parser usage on untrusted input | Tampering + remote code execution |
